...
class AgentPlan:
    ...
    def recommendation_summary(self) -> str:
        ...
        if self.seed_movie_name:
            parts.append(f"种子电影：{self.seed_movie_name}")
        if self.genres:
            parts.append(f"目标类型：{', '.join(self.genres)}")
        if self.excluded_genres:
            parts.append(f"排除类型：{', '.join(self.excluded_genres)}")
        if self.language:
            parts.append(f"语言：{self.language}")
        if self.min_rating is not None:
            parts.append(f"最低评分：{self.min_rating:.1f}")
        if self.personalized and self.user_id is not None:
            parts.append(f"个性化用户：{self.user_id}")
        return "；".join(parts) or "未提取到显式筛选条件"
...
class AgentPlanner:
    def plan(
        ...
        *,
        ...
        if genres or extracted_seed or personalized:
            recommendation_score += 2
        if any(keyword in lowered for keyword in {"推荐", "recommend", "类似", "想看"}):
            recommendation_score += 2
        ...
        if extracted_seed and personalized and parsed_user_id is not None:
            ...
            explanation = "结合种子电影和协同过滤信号扩展推荐。"
        elif extracted_seed:
            strategy = "content"
            explanation = "基于种子电影的内容相似度扩展推荐。"
        elif personalized and parsed_user_id is not None:
            strategy = "collaborative"
            explanation = "基于用户偏好和协同过滤信号生成推荐。"
        else:
            strategy = "local"
            explanation = "根据本地目录中的类型、语言和评分条件筛选推荐。"
    ...
    def _extract_min_rating(message: str, lowered: str) -> float | None:
        match = re.search(r"([0-5](?:\.\d)?)\s*(?:分|rating|rated)", message)
        if match:
            return float(match.group(1))
        if any(keyword in lowered for keyword in {"高分", "评分高", "high rating", "strong ratings"}):
            return 4.0
        return None
